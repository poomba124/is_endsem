...
# Global ElGamal Encryption Keys
...
def _hash_message_to_int(msg: str, p: int):
    ...
    # Hash is always < 2^256. If p is 2048-bit, the full hash fits easily.
    msg_hash = bytes_to_long(hashlib.sha256(msg.encode('utf-8')).digest())

    # Ensure the hash is reduced modulo (p-1) for signature calculation,
    # or just use the hash value m directly if p is large enough, which it is.
    return msg_hash % (p - 1)


def verify_elgamal_signature(summary_str: str, signature: Tuple[int, int],
                             signing_pub_key: Tuple[int, int, int]) -> bool:
    """Verifies an ElGamal signature (r, s) against the message hash."""

    p, g, y_a = signing_pub_key  # y_a is the public key a^g mod p
    r, s = signature

    # Calculate hash of the message (m = H(M))
    m = _hash_message_to_int(summary_str, p)

    # Check 1: Signature components must be within valid range
    if not (0 < r < p) or not (0 < s < p - 1):
        return False

    # Verification Equation: g^m ?= y_a^r * r^s mod p

    # Left side: g^m mod p
    v1 = pow(g, m, p)

    # Right side: y_a^r * r^s mod p
    # y_a^r mod p
    y_a_r = pow(y_a, r, p)
    # r^s mod p
    r_s = pow(r, s, p)
    ...
    # If V1 equals V2, the signature is valid
    return v1 == v2
...
def handle_client(conn: socket.socket, addr: tuple):
    ...
    try:
        # --- 1. SEND ELGAMAL PUBLIC KEY (p, g, y_encryption) ---
        ...
        print("[Gateway] Sent ElGamal Encryption Public Key (p, g, y) to client.")
        ...
        signature_r = int(payload["signature_r"], 16)
        signature_s = int(payload["signature_s"], 16)
        signature = (signature_r, signature_s)

        # ElGamal Signing Public Key
        signing_pub_y_a = int(payload["signing_pub_y_a"], 16)
        # Use common p and g from encryption keys
        signing_pub_key = (ELGAMAL_P, ELGAMAL_G, signing_pub_y_a)
        ...
        # 4. ElGamal Digital Signature Verification
        is_valid = verify_elgamal_signature(summary_str, signature, signing_pub_key)
...
def start_server():
    ...
    print("=" * 75)
    print("                PAYMENT GATEWAY SERVER (ElGamal Decryptor & ElGamal Signer)")
    print("=" * 75)
    print(f"ElGamal Public Key (p, g, y_encrypt): {format_key_output(ELGAMAL_PUB)}")
