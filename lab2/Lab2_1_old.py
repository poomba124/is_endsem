...
def des_block(block, keys, encrypt=True):
    ...
    if not encrypt:
        keys = keys[::-1]
    for key in keys:
        right_expanded = permute(right, expansion)
        temp = xor(right_expanded, key)
        temp = sbox_substitution(temp)
        temp = permute(temp, pbox)
        temp = xor(left, temp)
        left = right
        right = temp
    combined = right + left
...
def encrypt(msg, key):
    ...
    for i in range(0, len(msg), 8):
        block = str_to_bin(msg[i:i+8])
        result += des_block(block, keys, encrypt=True)
    return result
...
def decrypt(ciphertext, key):
    ...
    for i in range(0, len(ciphertext), 64):
        ...
        result += bin_to_str(des_block(block, keys, encrypt=False))
    return unpad(result)
...
ciphertext_bin = encrypt(message, key)
ciphertext_hex = bin_to_hex(ciphertext_bin)
print("Encrypted (hex):", ciphertext_hex)

ciphertext_bin_back = hex_to_bin(ciphertext_hex)
decrypted = decrypt(ciphertext_bin_back, key)
